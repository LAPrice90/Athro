...
let dk          = deckKeyFromState();
let progressKey = 'progress_' + dk;          // read/write here
...
function markSeenNow(cardId){
  const today = FC_UTILS.getLocalISODate ? FC_UTILS.getLocalISODate() : new Date().toISOString().slice(0,10);
...
function showNudge(){
  const existing = viewEl && viewEl.querySelector('[data-nudge]');
  if(existing) return;
  const note = document.createElement('div');
  note.setAttribute('data-nudge', 'true');
  note.className = 'muted';
  note.textContent = 'Take a moment, then try the answer before moving on.';
  viewEl && viewEl.appendChild(note);
}
...
if (location.hash.startsWith('#/newPhrase') && typeof window.render === 'function') {
  setTimeout(() => window.render && window.render(), 0);
}
