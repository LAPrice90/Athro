...
const DECKS = [
  { id: 'welsh_phrases_A1', name: 'Welsh - A1 Phrases', count: 112 },
  { id: 'welsh_homework', name: 'Welsh Homework', count: 8 }
];
...
window.STATE = STATE;
window.STRUGGLE_CAP = STRUGGLE_CAP;
...
const routes = {
  ...
  worklist: () => window.renderWorkList ? window.renderWorkList() : renderPlaceholder('Today')(),
  review: query => window.renderReview ? window.renderReview(query) : renderPlaceholder('Flashcards')(),
...
async function fcGetTestQueueCount(){
  ...
  const due = (window.FC_SRS && FC_SRS.getDuePhrases)
...
window.go = go;
window.loadDeckRows = loadDeckRows;
window.loadProgress = loadProgress;
window.saveProgress = saveProgress;
window.loadAttemptsMap = loadAttemptsMap;
window.todayKey = todayKey;
window.initDeckPicker = initDeckPicker;
...
async function renderHome(){
  ...
  const allowance = (window.FC_UTILS && FC_UTILS.peekAllowance)
    ? FC_UTILS.peekAllowance()
    : { remaining: 0 };
...
async function renderPhraseDashboard(){
  ...
  const allowanceState = FC_UTILS.peekAllowance();
