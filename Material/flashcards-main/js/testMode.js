(function(){
  ...
  function equalsLoose(a,b){
    if (window.FC_UTILS && typeof FC_UTILS.equalsAnswerLoose === 'function') {
      return FC_UTILS.equalsAnswerLoose(a,b);
    }
  ...
  function scheduleCardReview(row, result){
    const prog = loadProgress(dk);
    if(!prog.seen) prog.seen = {};
    const existing = prog.seen[row.id] || {};
    const card = {
      ...existing,
      id: row.id,
      introducedAt: existing.introducedAt || existing.firstSeen || new Date().toISOString(),
      reviews: Array.isArray(existing.reviews) ? existing.reviews.slice() : [],
      interval: existing.interval || 1,
      ease: existing.ease || 2.5,
      dueDate: existing.dueDate
    };

    if(window.FC_SRS && FC_SRS.scheduleNextReview){
      FC_SRS.scheduleNextReview(card, result, { now: new Date(), grace: SETTINGS.graceMode });
    }else{
      card.reviews.push({ date: new Date().toISOString(), result });
    }

    prog.seen[row.id] = {
      ...existing,
      introducedAt: card.introducedAt,
      firstSeen: existing.firstSeen || card.introducedAt.slice(0, 10),
      lastSeen: FC_UTILS.getLocalISODate ? FC_UTILS.getLocalISODate() : new Date().toISOString().slice(0, 10),
      interval: card.interval,
      ease: card.ease,
      dueDate: card.dueDate,
      reviews: card.reviews
    };
    saveProgress(dk, prog);
    row.interval = card.interval;
    row.ease = card.ease;
    row.dueDate = card.dueDate;
    row.reviews = card.reviews;
    return card;
  }
  ...
  function handleSubmit(skip){
    ...
    const scheduled = scheduleCardReview(c, pass ? 'pass' : 'fail');
  ...
  function startDrill(card, initialInput){
    ...
    function copyStep(step, wrongVal){
      ...
      function submit(){
        ...
        const scheduled = scheduleCardReview(card, ok ? 'pass' : 'fail');
        recordEvent && recordEvent('review_logged', {
          cardId: card.id,
          result: ok ? 'pass' : 'fail',
          oldInterval: bInt,
